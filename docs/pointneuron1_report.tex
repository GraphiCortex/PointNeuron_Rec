...
The beast samples are not being avoided because they are inconvenient. They are
separated because the oracle-node test shows a real graph-stage failure mode:
the current PointNeuron1.0 geodesic tree builder can invent long bridges even
when the node candidates are already near the ground truth.
}
