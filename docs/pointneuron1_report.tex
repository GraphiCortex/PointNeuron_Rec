...
\texttt{run\_status} instead records diagnostic triage: PASS means no trigger in
the available evidence; FAIL means review is required, including unresolved or
...
four data-preprocessing, five mixed and 34 unknown. There are no exclusively
...
\midrule
3, 4, 6 & Preprocessing & Saved data audits record 17, 25 and 24 out-of-bounds SWC nodes. Aggregation's alignment guard exits with the recorded code 2; subprocess output itself is unavailable. \\
...
test require new evidence. The schema includes dataset identity and retains
missingness for future reuse, but no Allen ingestion or PointNeuron2.0 work is
