...
The promoted Gold166 result is strong inside this report's topology-evaluation
setting, but it is not directly comparable to the original PointNeuron paper's
Janelia-Fly ESA/DSA/PDS and point-distance precision/recall/F1 benchmark. The
report therefore treats paper comparison as contextual, not as a claim that this
implementation beats the published numbers.
}
...
ESA, DSA, PDS, precision, recall, and F1. In contrast, this reimplementation
currently reports Gold166 topology metrics: edge F1, bridge count, and reachable
edge fraction. Therefore the numbers should not be compared as if they were the
same benchmark.
...
paper-style geometric discrepancy metrics; lower is better. This project did
not yet reproduce those exact metrics on the paper's Janelia-Fly split.

Our strongest promoted result is:

\begin{center}
\begin{tabular}{lccc}
\toprule
This project & Edge F1 & Bridges & Reachable \\
\midrule
Guarded30 adaptive geodesic, Gold166 stable domain & approx. 0.8295 & approx. 1.444 & approx. 0.9504 \\
\bottomrule
\end{tabular}
\end{center}

If one only looks at the F1 values superficially, our stable-domain edge F1 of
about 82.95\% is numerically higher than the paper's reported reconstruction F1
of 71.70\%. However, this is not a valid claim of outperforming the paper. The
dataset is different, the test split is different, and the metric is different.
The paper's F1 is a spatial point-distance reconstruction metric; our F1 is an
edge/topology metric induced from selected graph nodes. The correct conclusion
is more modest:

\reportbox{Paper comparison conclusion}{
This project successfully reproduces the conceptual PointNeuron pipeline and
achieves strong topology results on the validated Gold166 stable domain, but it
does not yet demonstrate that it matches or exceeds the original paper's
reported Janelia-Fly benchmark. To make that claim, the next evaluation step
must run paper-compatible metrics, including precision, recall, F1, ESA, DSA,
and PDS, on a paper-compatible dataset split.
}
